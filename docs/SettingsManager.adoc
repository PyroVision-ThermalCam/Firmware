...
typedef struct {
    ...
    Settings_t Settings;      // Current settings in RAM
...
* *Blob Key*: `"settings"` - Contains the complete `Settings_t` structure
* *Version Key*: Reserved for future migrations
* *Config valid Flag*: `"config_valid"` - Indicates a valid configuration in the NVS
...
==== 1. Lepton Camera Settings (`Settings_Lepton_t`)
...
typedef struct {
    Settings_ROI_Type_t Type; // ROI type
    uint16_t x;               // X coordinate of top-left corner (display pixels)
    uint16_t y;               // Y coordinate of top-left corner (display pixels)
    uint16_t w;               // Width of the ROI (display pixels)
    uint16_t h;               // Height of the ROI (display pixels)
} Settings_ROI_t;

typedef struct {
    Settings_ROI_t ROI[4];                       // Camera ROIs
    Settings_Emissivity_t EmissivityPresets[128]; // Emissivity presets
    size_t EmissivityPresetsCount;                // Number of presets
} Settings_Lepton_t;
----

* *ROIs*: Up to 4 Regions of Interest (Spotmeter, Scene, AGC, Video Focus); configured via `Settings_ROI_t`
* *Emissivity Presets*: Predefined material emissivities (e.g. Concrete: 0.92, Skin: 0.98)

==== 2. WiFi Settings (`Settings_WiFi_t`)
...
==== 3. Provisioning Settings (`Settings_Provisioning_t`)
...
} Settings_Provisioning_t;
----

==== 4. Display Settings (`Settings_Display_t`)
...
==== 5. HTTP Server Settings (`Settings_HTTP_Server_t`)
...
==== 6. VISA Server Settings (`Settings_VISA_Server_t`)
...
==== 7. System Settings (`Settings_System_t`)
...
|`Settings_Manager_Setting_t` or NULL (see note below)
...
[source,cpp]
----
esp_err_t SettingsManager_Load(Settings_t *p_Settings);
----
...
1. Writes complete `Settings_t` structure as blob to NVS
...
[source,cpp]
----
Settings_Display_t display_settings;
...
2. Resets `"config_valid"` flag (allows reloading JSON defaults)
...
[source,cpp]
----
Settings_System_t system_settings;
                                   ...
                                   int32_t id, void* event_data)
{
    switch (id) {
        ...
        case SETTINGS_EVENT_WIFI_CHANGED:
            Settings_WiFi_t* wifi = (Settings_WiFi_t*)event_data;
            ESP_LOGI(TAG, "WiFi settings changed: SSID=%s", wifi->SSID);
            // Reconfigure WiFi...
            break;
            
        case SETTINGS_EVENT_DISPLAY_CHANGED:
            Settings_Display_t* display = (Settings_Display_t*)event_data;
...
{
    Settings_WiFi_t old_settings;
    Settings_WiFi_t new_settings;
...
1. *JSON Config* (`data/default_settings.json`)
   ...
   * Flag `"config_valid"` prevents reloading of the configuration into the Flash memory
...
2. Flash firmware
3. Perform factory reset or manually reset `config_valid` flag
...
*Cause*: `config_valid` flag is set
...
nvs_set_u8(handle, "config_valid", 0);
nvs_commit(handle);
nvs_close(handle);
esp_restart();
----
